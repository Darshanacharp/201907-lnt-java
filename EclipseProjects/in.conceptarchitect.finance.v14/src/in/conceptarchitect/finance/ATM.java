...
public class ATM {
	...
	private void adminOpenAccount() {
		...
		String type=keyboard.readString("Account Type [S.Savings/C.Current/O.OD]?").toLowerCase().substring(0,1);
		String name=keyboard.readString("Name?");
		String password=keyboard.readString("password?");
		double amount=keyboard.readDouble("amount?");
		if(type.equals("c"))
			bank.openCurrentAccount(name, password, amount);
		else if (type.equals("o"))
			bank.openOverdraftAccount(name, password, amount);
		else
			bank.openSavingsAccount(name, password, amount);
